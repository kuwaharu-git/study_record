...
@check_error
def main(user_id):
    year_month = input_util.input_year_month(
        "グラフを作成する年、月を入力してください['%Y-%m']"
    )
    ...
    rows = access_study_records.get_records(
        user_id, year_month.date(), year_month.date().replace(day=last_date)
    )
    # HTMLの作成
    with open(
        f"{os.path.dirname(os.path.abspath(__file__))}/{user_id}_output.html",
        mode="w",
        encoding="utf-8",
        newline="\n",
    ) as f:
        ...
        f.write("</body>\n")
        f.write(
            f'<img src="{os.path.dirname(__file__)}/{user_id}_stacked_bar_graph.png">\n'
        )
        ...
        for row in rows:
            f.write("<tr>\n")
            f.write(
                f'<td>{row["id"]}</td><td>{row["category_name"]}</td><td>{row["study_date"]}</td><td>{row["study_time"]}</td>\n'
            )
            f.write("</tr>\n")
        f.write("</table>\n")
        f.write("</body>\n")
